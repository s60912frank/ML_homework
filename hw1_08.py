...
for zz in range(100):
    ...
    with tf.Session() as sess:
        ...
        for i in range(1000):
            ...
            avg_err_rate = err_rate / len(x_bat) * 0.1
            print(avg_err_rate)
            ##Early stopping
            if last_err_rate < avg_err_rate:
                ...
                best_record['err_rate'] = avg_err_rate
                best_record['time'] = i
                best_record['weights'] = sess.run(w).reshape((1, 8))
                best_record['bias'] = sess.run(b)
                raising_count = 0
        ...
        print("========================")"""
        #print(str(best_record))
        #tf.summary.FileWriter("logs/", tf.get_default_graph().as_graph_def())
        #show_graph(tf.get_default_graph().as_graph_def())
        file.write(str(best_record['time']) + ', ' + str(best_record['err_rate']) + ', ' + str(best_record['weights']) + ', ' + str(best_record['bias']) + '\n')
    ###### Start TF session ######
file.close()
